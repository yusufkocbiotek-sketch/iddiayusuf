...
def generate_story(match, verbose=False):
    ...
    if ms1 == 99.0 or (ust25 == 99.0 and ust35 == 99.0 and ust15 == 99.0):
    ...
    def rule_sort_key(r):
        ...
        # Matematiksel kesinlik barındıran kilit kuralları en yüksek önceliklidir (Faz 0)
        if 'paradoks' in name or 'matematiksel' in name: return 0 
        ...
        if code.startswith('T'): return 3 # Faz 3 (Taraf)
        if code.startswith('G'): return 4 # Faz 4 (Gol)
    ...
    if not triggered_rules:
        ...
        for r in triggered_rules:
            output.append(f"| **{r['code']}** | {r['name']} | {r['category']} | ✅ **AKTİF** - {r['insight']} |")
