...
for mac in matches:
    ...
    ev_15_alt = od.get('Ev Sahibi Alt/Üst 1.5_Alt', 99)
    dep_15_alt = od.get('Deplasman Alt/Üst 1.5_Alt', 99)
    
    sev = mac.get('skor_ev', -1)
    sdep = mac.get('skor_dep', -1)
    
    if alt25 <= 1.55 and ms0 <= 2.85 and ev_15_alt <= 1.35 and dep_15_alt <= 1.35 and sev >= 0:
        toplam_gol = sev + sdep
        found.append({
            'skor': f"{sev}-{sdep}",
...
print(f"Toplam Takim 1.5 Alt Kosullu 1486 Mac: {len(found)}")
if found:
