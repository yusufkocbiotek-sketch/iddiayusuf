...
class Rule1486(BaseRule):
    ...
    category = "GOL"
    description = "MS0 <= 2.85, 2.5 Alt <= 1.57 ve İlk Yarı 0/0 <= 4.00"
    @classmethod
    def evaluate(cls, odds):
...
class Rule1558(BaseRule):
    code = "1558"
    category = "SKOR"
    name = "Aşırı Şişirilmiş Üst Tuzağı (Dengeli Maç Paradoksu)"
    description = "Dengeli maçlarda (Taraf >= 1.65) 1.5 Üst <= 1.15 olması dev bir 2.5 Alt tuzağıdır."
    ...
    def evaluate(cls, odds):
        ...
        if ust_15 <= 1.15 and ust_15 != 99.0 and ms1 >= 1.65 and ms2 >= 1.65 and ms1 != 99.0 and ms2 != 99.0:
            return True, "AŞIRI ŞİŞİRİLMİŞ ÜST TUZAĞI (DENGELİ MAÇ PARADOKSU): Taraf oranları birbirine yakın/dengeli olmasına rağmen (1.65 üzeri), 1.5 Üst oranı (<= 1.15) adeta 'Bu maçta kesinlikle gol yağmuru olacak' diye bağırıyor. Bu İddaa'nın en klasik psikolojik illüzyonlarından biridir. Ağır bir favori olmadığı halde Üst oranı bu kadar düşük tutuluyorsa, maç yüksek ihtimalle tamamen kilitlenecek ve 2.5 Alt bitecektir. Herkes gollere koşarken kısır bir maç izleyeceğiz!"
        return False, ""
...
class RuleY11(BaseRule):
    ...
    @classmethod
    def evaluate(cls, odds):
        cs12 = get_odd(odds, ["Çifte Şans_12", "Çifte Şans_1 ve 2"])
        kg_var = get_odd(odds, ["Karşılıklı Gol_Var", "Karşılıklı Gol Var"])
        ...
        if ms1 >= 1.55 and ms2 >= 1.55 and ms1 != 99.0 and ms2 != 99.0:
...
class RuleT70(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.45 and ms0 <= 3.20 and ms1 != 99.0 and ms0 != 99.0:
...
class Rule1472(BaseRule):
    ...
    category = "GOL_VE_YÖN"
    description = "MS0 <= 2.85, 2.5 Alt <= 1.57 AMA Deplasman (Sürpriz) 0.5 Üst <= 1.30"
...
class Rule1477(BaseRule):
    ...
    description = "MS1 <= 1.30 + Ev 2.5 Alt <= 1.57 + Dep 0.5 Üst <= 1.50"
...
class Rule1505(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms0 != 99.0 and ms0 > 0 and ms0 <= 2.70 and ms1 >= 2.15 and ms2 >= 2.15:
...
class Rule1518(BaseRule):
    ...
    description = "Ev 0.5 Üst <= 1.35 + Dep 0.5 Üst <= 1.35 + (2.5 Alt <= 1.57 veya KG Yok <= 1.70) = KG Var / Üst"
...
class Rule1534(BaseRule):
    ...
    def evaluate(cls, odds):
        ms1 = get_odd(odds, ["Maç Sonucu_1", "Maç Sonucu_Ev", "1"])
        ms0 = get_odd(odds, ["Maç Sonucu_0", "Maç Sonucu_Beraberlik"])
        ev_15_alt = get_odd(odds, ["Ev Sahibi Alt/Üst 1.5_Alt", "Ev Sahibi Altı/Üstü 1.5_Alt"])
        kg_yok = get_odd(odds, ["Karşılıklı Gol_Yok", "Karşılıklı Gol Yok"])
...
class Rule1530(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        # Eğer ev sahibi net favori ise (MS1 < 1.95), bu kural çalışmamalıdır çünkü o maçlar gerçekten kısır geçebilir (1-0 gibi).
...
class Rule1554(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 != 99.0 and ms1 <= 1.25 and ms0 != 99.0 and ms0 <= 4.40 and ms2 != 99.0 and ms2 <= 7.50:
...
class Rule1487(BaseRule):
    ...
    description = "MS favorisi <= 1.50 iken 2.5 Alt <= 1.57 ise, bu maç 2.5 Üst veya Üst patlaması yapar."
    @classmethod
    def evaluate(cls, odds):
        ...
        # Biri ağır favoriyse
        if (ms1 <= 1.35 or ms2 <= 1.35) and 1.45 <= alt25 <= 1.57 and alt25 != 99.0:
