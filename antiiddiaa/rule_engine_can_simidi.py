...
def get_odd(odds, keys, default=99.0):
    for k in keys:
        ...
        if "Çifte Şans_12" in k or "Çifte Şans_1-2" in k or "Çifte Şans_1 ve 2" in k:
            for alt_k in ["Çifte Şans_12", "Çifte Şans_1-2", "Çifte Şans_1 ve 2", "-_1 ve 2"]:
                if alt_k in odds: return odds[alt_k]
                
        if "Çifte Şans_1X" in k or "Çifte Şans_1-0" in k or "Çifte Şans_1 ve 0" in k:
            for alt_k in ["Çifte Şans_1X", "Çifte Şans_1-0", "Çifte Şans_1 ve 0", "-_1 ve 0"]:
                if alt_k in odds: return odds[alt_k]
                
        if "Çifte Şans_X2" in k or "Çifte Şans_0-2" in k or "Çifte Şans_0 ve 2" in k:
...
class Rule1485(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.55 and alt25 <= 1.65 and cs12 > 1.20 and cs12 < 99.0:
            return True, "ÖLÜ ALT TUZAĞI: Ev sahibi net favori, 2.5 Alt oranı çok düşük. Normalde bu maçın 1-0 veya 2-0 bitmesi beklenir. Ancak Çifte Şans 12 oranı şüpheli derecede yüksek (1.20+). Bu durum maçta bir beraberlik potansiyeli (1-1) veya sürpriz gollü bir Deplasman reaksiyonu (2-1, 2-2) olduğunu gösterir. Maç kesinlikle Alt bitmeyecektir; KG Var ve Üst denenmeli."
        return False, ""
...
class Rule1488(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.25 and ev_25_alt <= 1.50:
...
class RuleY8(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if iy_kg_var > 0 and iy_kg_var <= 2.90:
...
class RuleY9(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if iy_kg_var > 0 and iy_kg_var <= 2.90 and ikiy_kg_var > 0 and ikiy_kg_var <= 2.30:
...
class RuleT28(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms2 != 99.0 and ms2 <= 1.50 and ev_05_ust != 99.0 and ev_05_ust <= 1.60:
             return True, "DEPLASMAN FAVORİSİ (ZAYIF TAKIM GOL TUZAĞI): Deplasman takımı 1.50 ve altı oranla çok net favori. Ancak zayıf ev sahibinin (5.00+ oranlı takımın) gol atma oranı (0.5 Üst) 1.60 ve altında! Zayıf takımın bu kadar net gol atacağı düşünülüyorsa, deplasman takımının kazanma ihtimali riske girmiş demektir. MS1 veya 1X sürprizi denenmelidir."
...
class RuleT31(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if (ms1 <= 1.30 or ms2 <= 1.30) and kg_var <= 1.45 and (ust35 > 1.80 or ust35 == 99.0):
...
class RuleT32(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.60 and dep_05_ust <= 1.60 and cift <= 1.65 and alt35 <= 1.30:
...
class RuleT33(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.45 and dep_05_ust <= 1.55 and kg_yok <= 1.65:
            return True, "Matematiksel Paradoks! Ev sahibi ağır favori, deplasmanın da gol atması bekleniyor (0.5 Üst). Eğer favori kazanacaksa ve zayıf takım gol atacaksa maç KG Var olmalı! Ancak bahis şirketleri KG Yok seçeneğini düşük tutmuş. Bu çelişki, favori takımın (Ev) maçı kazanamayacağının (0-0 veya Deplasman galibiyeti) matematiksel itirafıdır!"
            
        # Deplasman favori, ama Ev gol atar ve KG Yok
        if ms2 <= 1.45 and ev_05_ust <= 1.55 and kg_yok <= 1.65:
            return True, "Matematiksel Paradoks! Deplasman ağır favori, ev sahibinin de gol atması bekleniyor (0.5 Üst). Eğer favori kazanacaksa ve zayıf takım gol atacaksa maç KG Var olmalı! Ancak bahis şirketleri KG Yok seçeneğini düşük tutmuş. Bu çelişki, favori takımın (Deplasman) maçı kazanamayacağının (0-0 veya Ev galibiyeti) matematiksel itirafıdır!"
...
class RuleT35(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 >= 2.30 and ms2 >= 2.30 and abs(ms1 - ms2) <= 0.30 and cs_12 < 99.0 and cs_12 <= 1.20 and kg_var >= 1.65:
...
class RuleY3(BaseRule):
    ...
    def evaluate(cls, odds):
        iy2_kgyok = get_odd(odds, ["2. Yarı Karşılıklı Gol_Yok"])
        iy_00 = get_odd(odds, ["1. Yarı Skoru_0:0"])
        if iy2_kgyok <= 1.20 and iy_00 >= 2.50:
...
class RuleY5(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        iy_15_alt = get_odd(odds, ["1. Yarı Alt/Üst 1.5_Alt", "1. Yarı Altı/Üstü 1.5_Alt"])
        iy2_kgyok = get_odd(odds, ["2. Yarı Karşılıklı Gol_Yok"])
        if iy_05_ust <= 1.35 and iy_15_alt <= 1.30 and iy2_kgyok <= 1.10:
...
class RuleG11(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if kg_var <= 1.35 and kg_var < 99.0 and ms1 > 1.40 and ms2 > 1.40:
...
class Rule1462B(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        is_kg_yok_favori = kg_yok <= 1.42 and kg_var >= 2.0
...
class Rule1475(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.20:
            if (ev_15_alt < ev_15_ust) or (ev_25_alt < ev_25_ust and kg_var <= 1.65):
                is_trap = True
                
        if ms2 <= 1.20:
            if (dep_15_alt < dep_15_ust) or (dep_25_alt < dep_25_ust and kg_var <= 1.65):
                is_trap = True
...
class Rule1499(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 2.20 and ms2 >= 2.90 and ev_alt15 > 0 and ev_alt15 <= 1.45 and kg_yok > 1.45:
...
class Rule1500(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if 1.40 <= ms1 <= 1.65 and kgy >= 1.95 and kg_var <= 1.50 and ust <= 1.55:
...
class Rule1502(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 > 0 and h01_1 > 0 and ms1 <= 1.55 and h01_1 <= 2.15 and alt25 <= 1.65:
...
class Rule1504(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 > 0 and ms1 <= 1.35 and ev_ust15 > 0 and ev_ust15 >= 1.45 and ev_ust15 != 99.0:
...
class Rule1507(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 > 0 and ms1 <= 1.45 and h01_2 > 0 and h01_2 <= 1.95:
...
class Rule1508(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if (ms1 > 0 and ms1_kg_yok > 0 and ms1_kg_var > 0 and dep_05_ust > 0 and kg_var > 0):
            if ms1 <= 1.65 and kg_var >= 1.60 and ms1_kg_yok < ms1_kg_var and dep_05_ust <= 1.40:
...
class Rule1512A(BaseRule):
    ...
    category = "GOL"
    description = "Ağır Favori maçında 2.5 Alt/Üst baremi açılmamışsa ve 3.5 Üst < 1.80 ise 4-5 gol olur."
    @classmethod
    def evaluate(cls, odds):
        ms1 = get_odd(odds, ["Maç Sonucu_1"])
        kg_var = get_odd(odds, ["Karşılıklı Gol_Var"])
...
class Rule1512B(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if not has_25 and has_35 and 0 < ms1 <= 1.55 and 0 < kg_var <= 1.50 and ust35 > 0:
            if not (ust35 < 1.80 or (ev_iy_05_ust > 0 and ev_iy_05_ust <= 1.50) or (dep_iy_05_ust > 0 and dep_iy_05_ust <= 1.50)):
...
class Rule1513(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        # ⛔ KRİTİK İSTİSNA (Kapa-KTP tipi hatayı önler):
        # İY KG Yok <= 1.12 VEYA 2Y KG Yok <= 1.25 ise maç GERÇEKTEN tek taraflıdır.
        # Bu durumda 1513 (suni favori = ev gol atar) YANLIŞ olur → 1513B devralır.
        iy_kg_yok = get_odd(odds, ["İlk Yarı Karşılıklı Gol_Yok"])
        ...
        if 1.30 <= ms2 <= 1.55 and h2 >= 2.80 and 0 < kg_var <= 1.55:
            return True, "SUNİ FAVORİ VE DÜELLO: Deplasman takımı kağıt üzerinde ağır favori. Ancak Handikap 2 oranı (2.80 ve üzeri) iddaa'nın deplasmanın maçı rahat kazanacağına inan olmadığını gösteriyor. Üstelik KG Var oranının düşük olması, Ev sahibinin kesinlikle gol/goller bulacağını kanıtlıyor. Bu maçta Ev Sahibi sürprizi (1X Çifte Şans) ve Gollü bir senaryo (3-2, 2-2, 2-1) yaşanacaktır."
...
class Rule2003(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if is_guclu_fav and is_15_ust_kucuk and is_dengeli_25:
            return True, (
                "MİLLİ MAÇ GÜÇlü FAVORİ GOL ŞOVU: Ev sahibi (MS1 <= 1.52) son derece güçlü bir favori. "
                "1.5 Üst oranı " + str(ust15) + " ile piyasa zaten 2+ gol neredeyse garantilemekte. "
                "2.5 Alt/Üst piyasası ise inanılmaz derecede dengeli açılmış (fark: " + str(round(abs(alt25-ust25),2)) + "). "
                "Bu 'dengeli 2.5' görünümü bir ALDATMACADIR! Milli maçlarda bu kadar güçlü bir ev favorisi "
                "zayıf rakibi fark atar. 2.5 ÜSTÜ, 3.5 Üstü ve Ev MS1 bankodur. "
                "Verilen örnekte Hollanda 5-1 kazandı ve KG Var gerçekleşti (2.5 Üst doğru tahmin)."
            )
        return False, ""
...
class Rule1514(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        is_balanced = ms1 != 99.0 and ms2 != 99.0 and min(ms1, ms2) >= 1.70 and max(ms1, ms2) <= 2.80
        
        if 0 < kg_var <= 1.35 and iy_15_ust >= 2.60 and iy_15_ust != 99.0 and is_balanced:
...
class Rule1515(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms2 <= 1.50 and ms1 >= 4.00:
            # KRİTİK FİLTRE: KG Var 1.35'in altındaysa bu bir tuzak değil, GERÇEK bir düellodur (örn: 2-5).
            # Ayrıca Ev sahibinin gol atma oranı çok düşükse (1.45 altı) gerçekten gol atar.
            ev_05_ust = get_odd(odds, ["Ev Sahibi Alt/Üst 0.5_Üst", "Ev Sahibi Altı/Üstü 0.5_Üst"])
            
            if 1.35 <= kg_var <= 1.50 and (ev_05_ust == 99.0 or ev_05_ust >= 1.45):
                if dep_15_ust != 99.0 and dep_15_ust > 1.55:
                    return False, ""
                    
                return True, "SAHTE EV SAHİBİ GOLÜ: Deplasman takımı rahat kazanacak (1.50 altı). Ev sahibine ise kazanması imkansız bir oran (4.00 ve üzeri) açılmış. Buna rağmen KG Var oranı 1.50 civarında 'cazip' bir tuzak seviyesinde. Bu, iddaacıların 'Ev sahibi nasıl olsa evinde 1 gol atar' diye düşünmesini sağlamak için kurulmuş devasa bir yemdir. Ev sahibi gol atamaz, Deplasman takımı fark atarak (0-3, 0-4) kazanır. KG Yok."
        return False, ""
...
class Rule1516(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.45 and ms2 >= 4.00:
            dep_05_ust = get_odd(odds, ["Deplasman Alt/Üst 0.5_Üst", "Deplasman Altı/Üstü 0.5_Üst"])
            
            # KG Var çok düşükse (1.35 altı) veya Deplasman 0.5 Üst 1.45 altındaysa gerçekten atar! (örn 2-2 biter)
            if 1.35 <= kg_var <= 1.50 and (dep_05_ust == 99.0 or dep_05_ust >= 1.45):
                if ev_15_ust != 99.0 and ev_15_ust > 1.55:
                    return False, ""
                    
                return True, "SAHTE DEPLASMAN GOLÜ: Ev sahibi takımı rahat kazanacak (1.45 altı). Deplasmana ise kazanması imkansız bir oran (4.00 ve üzeri) açılmış. Buna rağmen KG Var oranı 1.50 civarında tuzak seviyesinde. Bu, iddaacıların 'Ev sahibi zaten yener, Deplasman da bari 1 gol atar' diye düşünmesini sağlamak için kurulmuş devasa bir yemdir. Deplasman gol atamaz, Ev Sahibi takımı fark atarak (3-0, 4-0) kazanır. KG Yok."
        return False, ""
...
class Rule1518(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ev_05_ust <= 1.35 and dep_05_ust <= 1.35 and (alt_25 <= 1.55 or kg_yok <= 1.75):
            return True, f"GİZLİ DÜELLO (PİYASA MANİPÜLASYONU): Ana marketlere bakan halk 2.5 Alt ve KG Yok oranlarını görüp maçın kısır geçeceğine inanıyor. ANCAK alt marketler gerçeği söylüyor: Ev sahibi 0.5 Üst ve Deplasman 0.5 Üst çok düşük açılmış. Yani bürolar her iki takımın da kesinlikle gol atacağını biliyor! Bu inanılmaz bir tuzaktır. Maç 0-0 veya kısır bitmeyecek, tam aksine 1-1'i çok erken aşıp 3-1, 1-2 gibi gollü skorlara gidecek. Hedef KG Var ve Üst!"
...
class Rule1531(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if ms1 <= 1.85 and dep_05_ust <= 1.35 and dep_05_ust != 99.0:
...
class Rule1530(BaseRule):
    ...
    category = "ZAMANLAMA"
    description = "2.5 Alt banko iken İlk Yarı 0.5 Üst çok düşük açılmış."
    ...
    def evaluate(cls, odds):
        ...
        if alt25 <= 1.60 and iy_05_ust <= 1.35 and iy_05_ust != 99.0:
            return True, "İLK YARI ŞİFRESİ (ERKEN GOL PATLAMASI): Piyasa 2.5 Alt (1.60 altı) göstererek maçı kısır bir 0-0 veya 1-0 maçına kilitliyor. FAKAT mikro baremlere baktığımızda, İlk Yarı 0.5 Üst (1.35 altı) adeta banko gösterilmiş! İddaa ilk yarıdan kesinlikle en az bir gol geleceğini biliyor. Genel kısır algısının aksine, bu maç ilk yarıdan çözülecek ve büyük ihtimalle 2.5 Üst'e de taşınacaktır. 0-0 kilitlenmesi bekleyenler büyük hüsrana uğrayacaktır. İlk yarı golü değerlendirilmelidir."
        return False, ""
...
class Rule1519(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if 1.01 < min(ms1, ms2) <= 1.30 and alt_35 <= 1.50 and alt_35 != 99.0:
            return True, "KISIR ŞOVMEN TUZAĞI: Ağır favorinin olduğu bir maçta (1.30 altı oran), İddaa'nın 3.5 Alt oranını bu kadar düşük (1.50 altı) açması büyük bir çelişkidir. Normalde favorinin şov yapması beklenir ama piyasa maçta 4 gol olmayacağından emin. Bu takım hücumda zorlanacak ve maç kilitlenecektir. Hatta sürpriz bir 0-0 bile çıkabilir. MS bahsinden uzak durulmalı, 3.5 Alt veya KG Yok oynanmalıdır."
        return False, ""
...
class Rule1482(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if kg_var <= 1.40 and ust25 <= 1.50 and ms0 <= 3.15 and ms1 < ms2 and ms1 >= 1.80:
...
class Rule1536(BaseRule):
    ...
    def evaluate(cls, odds):
        ...
        if alt25 == 99.0 and kg_var <= 1.45 and ust35 <= 1.60 and kg_var != 99.0 and ust35 != 99.0:
...
def evaluate_synergies(triggered_codes):
    """
    Tetiklenen kuralların kodlarını (list of strings) alır ve 
    eğer kesişen bir mega sinerji varsa mesajları liste olarak döndürür.
    """
    synergies = []
    
    y11_var = "Y11" in triggered_codes
    y1_var = "Y1" in triggered_codes
    y2_var = "Y2" in triggered_codes
    
    ms2_var = any(code in triggered_codes for code in ["1503", "1535"])
    ms1_var = any(code in triggered_codes for code in ["1516", "T8"])
    
    # 0/2 Sinerjisi (İY Kilit, Deplasman Koparır)
    if y11_var and ms2_var:
        synergies.append("🚨 MEGA SİNERJİ [0/2]: Y11 (İY 0 Bankosu) + 1503/1535 (Deplasman Sürprizi) aynı anda yandı! İlk yarı kilitlenecek ancak maçı deplasman koparacak! (Tahmini Oran: 10.00+)")
        
    # 0/1 Sinerjisi (İY Kilit, Ev Sahibi Koparır)
    if y11_var and ms1_var:
        synergies.append("🚨 MEGA SİNERJİ [0/1]: Y11 (İY 0 Bankosu) + MS1 Kuralı aynı anda yandı! İlk yarı kilitlenecek ancak maçı ev sahibi koparacak! (Tahmini Oran: 5.00+)")
        
    # 1/1 Sinerjisi (Ev Sahibi Dominasyonu)
    if y1_var and ms1_var:
        synergies.append("🚨 MEGA SİNERJİ [1/1]: Y1 (Ev Sahibi İY Atar) + MS1 Kuralı! Ev sahibi ilk yarıdan fişi çekecek! (Tahmini Oran: 2.50+)")
        
    # 2/2 Sinerjisi (Deplasman Dominasyonu)
    if y2_var and ms2_var:
        synergies.append("🚨 MEGA SİNERJİ [2/2]: Y2 (Deplasman İY Atar) + 1503/1535! Deplasman ilk yarıdan fişi çekecek! (Tahmini Oran: 6.00+)")
        
    # 1/2 Sinerjisi (Efsanevi Geri Dönüş)
    if y1_var and ms2_var:
        synergies.append("🚨 MEGA SİNERJİ [1/2]: Y1 (Ev Sahibi İY Atar) + 1503/1535 (Deplasman Sürprizi)! Ev sahibi ilk yarı öne geçecek ama Deplasman efsanevi bir geri dönüş yapacak! (Tahmini Oran: 25.00 - 35.00)")
        
    # 2/1 Sinerjisi (Efsanevi Geri Dönüş)
    if y2_var and ms1_var:
        synergies.append("🚨 MEGA SİNERJİ [2/1]: Y2 (Deplasman İY Atar) + MS1 Kuralı! Deplasman öne geçecek ama Ev Sahibi efsanevi bir geri dönüş yapacak! (Tahmini Oran: 25.00 - 35.00)")
