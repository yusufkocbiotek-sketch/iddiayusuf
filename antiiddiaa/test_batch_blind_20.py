...
def analyze_batch():
    ...
    for match in batch:
        ...
        for line in lines:
            ...
            if "DOMİNANT KURAL" in line:
                dominant_tahmin = line.strip()
            if "**Ana Tahmin:**" in line:
                ana_tahmin = line.strip()
            if "**Kombine Öneri:**" in line:
                kombine_oneri = line.strip()
                
        report_lines.append(f"{kural_sirasi}")
        report_lines.append(f"{genel_profil}")
        report_lines.append(f"{dominant_tahmin}")
        report_lines.append(f"{ana_tahmin}")
        report_lines.append(f"{kombine_oneri}")
