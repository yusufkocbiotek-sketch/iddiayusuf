...
# İlk hedef 2 adım; yetmezse 6’ya kadar çıkar
HARVEST_STEPS_FIRST = 2
HARVEST_STEPS_MAX = 6

FIND_MATCH_MAX_SCROLLS = 14     # maç ararken aşağı doğru kaç adım tarayalım
FIND_SCROLL_PX = 1200
...
def parse_card_to_match(card):
    ...
    if len(lines) < 2:
        return None
    # çok satır varsa, ilk iki takımı al
...
def saat_mi(text):
    if isinstance(text, str) and len(text) == 5 and text[2] == ":":
        try:
            s, d = text.split(":")
            return 0 <= int(s) <= 23 and 0 <= int(d) <= 59
        except:
            return False
    return False
...
def detay_parse(driver):
    ...
    while i < len(lines) - 1:
        ...
        # market ismi gibi davran
        if not nokta_var_mi(line) and not saat_mi(line):
            current_market = line
...
def click_match_by_text(driver, ev, dep):
    """
    ÇALIŞAN scraperyusuf mantığı:
    card.text içinde ev ve dep geçiyorsa tıkla.
    """
    init_scroll_target(driver)
    reset_scroll_top(driver)
...
def main():
    print("=" * 70)
    print("⚽ IDDAA SOFT30 V3 (deneme) - çalışanı bozmaz")
    ...
    try:
        ...
        for idx, m in enumerate(match_list, 1):
            ...
            ok = click_match_by_text(driver, ev, dep)
            if not ok:
                print("   ❌ tıklanamadı")
                fail += 1
                continue
            ...
            # lazy-load tetik
            driver.execute_script("window.scrollTo(0, 600);"); time.sleep(0.8)
            driver.execute_script("window.scrollTo(0, 1200);"); time.sleep(0.8)
            ...
            # saat yakala
            mac_saat = ""
            for t in re.findall(r"\b\d{1,2}:\d{2}\b", body):
                if len(t) == 5:
                    mac_saat = t
                    break
