...
# 🧠 YENİ EKLENDİ: Akıllı İsim Algılama ve Sözlük
def normalize_key(takim_adi, _=""):
    if not takim_adi: return ""
    s = str(takim_adi).lower().strip()
    
    # 🔴 İddaa ile SporDB arasındaki kronik isim farkları (K Kyzylorda = Kaisar)
    alias_dict = {
        "aktobe lento": "aktobe",
        "k kyzylorda": "kaisar",
...
# =========================
# ⏰ TARİH TOLERANS FONKSİYONU (±2 GÜN)
# =========================
...
# =========================
# 🧠 EŞLEŞTİRME (±2 GÜN KORUMALI + AKILLI İSİM EŞLEŞTİRME)
# =========================
def merge_data(mevcut, yeni):
    mac_listesi = mevcut.get("matches", [])
    guncelle_say = 0
    ekle_say = 0
    print("\n🔎 EŞLEŞTİRME BAŞLADI (±2 Gün Toleranslı & Akıllı İsim Algılama Aktif)...")

    for sp in yeni:
        eslesti = False
        for idx, mevcut_mac in enumerate(mac_listesi):
            # 1. KURAL: ±2 Gün Tarih Kontrolü
            if not tarih_farki_uygun_mu(mevcut_mac.get("tarih", ""), sp.get("tarih", "")): 
                continue
                
            # 2. KURAL: İsim Benzerliği
            n_mevcut_ev = normalize_key(mevcut_mac["ev_sahibi"], "")
            n_yeni_ev = normalize_key(sp["ev_sahibi"], "")
            
            n_mevcut_dep = normalize_key(mevcut_mac["deplasman"], "")
            n_yeni_dep = normalize_key(sp["deplasman"], "")
            
            oran_ev = SequenceMatcher(None, n_mevcut_ev, n_yeni_ev).ratio()
            oran_dep = SequenceMatcher(None, n_mevcut_dep, n_yeni_dep).ratio()

            # 🧠 YENİ EKLENDİ: Kapsama Mantığı (Biri diğerinin içinde tam olarak geçiyorsa Oranı %90 yap!)
            if len(n_mevcut_ev) > 3 and len(n_yeni_ev) > 3:
                if n_mevcut_ev in n_yeni_ev or n_yeni_ev in n_mevcut_ev:
                    oran_ev = max(oran_ev, 0.90)
                    
            if len(n_mevcut_dep) > 3 and len(n_yeni_dep) > 3:
                if n_mevcut_dep in n_yeni_dep or n_yeni_dep in n_mevcut_dep:
                    oran_dep = max(oran_dep, 0.90)

            # EŞLEŞTİRME BAŞARILI MI?
            if (oran_ev >= ESLESME_ORANI and oran_dep >= ESLESME_ORANI):
                ...
                if degisti_mi:
                    ...
                    guncelle_say += 1
                    print(f"🔄 GÜNCELLE : {mevcut_mac['ev_sahibi']} - {mevcut_mac['deplasman']} | Oran: %{int(oran_ev*100)} - %{int(oran_dep*100)}")
                eslesti = True
                break
        ...
        if not eslesti:
            ...
            ekle_say += 1
            print(f"➕ YENİ KAYIT: {sp['ev_sahibi']} - {sp['deplasman']} ({sp.get('tarih')})")
...
if __name__ == "__main__":
    print("="*60)
    print("🚀 SPORDB | STALE HATASI ÇÖZÜLDÜ | ±2 GÜN TOLERANS | AKILLI İSİM ZEKASI ✅")
    print("="*60)
