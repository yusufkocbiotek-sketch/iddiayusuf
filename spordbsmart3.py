...
# Bana liste/SS attıkça buraya yeni satırlar ekleyeceğiz.
# Format:  "sitedeki/spordb'deki isim" : "senin json'daki isim"
# Hepsi küçük harf yazılır, bot otomatik küçültüp bakar.
# =========================================================
TAKIM_TAKMA_ADLAR = {
...
def isim_normalize(s):
    """İsmi küçült, takma adlar sözlüğünden geçir, temizle."""
    if not s:
        return ""
    s = str(s).lower().strip()

    # Önce takma ad sözlüğüne bak (tam eşleşme)
    ...
    # Gereksiz ekleri at
    for ek in [" fc", " fk", " sk", " jk", " cf", " sc", " club", " u19", " u21", " u23"]:
        if s.endswith(ek):
            s = s[: -len(ek)]
...
def merge_data(mevcut, yeni):
    ...
    ekle_say = 0
    atlanan_say = 0
    print("\n🔎 EŞLEŞTİRME BAŞLADI (EN İYİ ADAY | ±3 GÜN | HAFIZA AKTİF)...")

    # Bir JSON maçı sadece BİR web verisiyle eşleşebilsin
    ...
    for sp in yeni:
        ...
        # ─────────────────────────────────────────────
        # 1️⃣ TÜM ADAYLARI PUANLA (ilk bulduğuna yazma!)
        # ─────────────────────────────────────────────
        en_iyi_index = -1
        en_iyi_puan = 0.0
        en_iyi_detay = ""
        ...
        for idx, mevcut_mac in enumerate(mac_listesi):
            if idx in kullanilan_indexler:
                continue  # Bu JSON maçı zaten eşleşti, tekrar yazma

            # Tarih kontrolü (±3 gün tolerans)
            if not dates_close(mevcut_mac.get("tarih", ""), sp_tarih):
                continue

            # Benzerlik puanları (hafıza sözlüğünden geçmiş halleriyle)
            oran_ev = benzerlik(mevcut_mac.get("ev_sahibi", ""), sp_ev)
            oran_dep = benzerlik(mevcut_mac.get("deplasman", ""), sp_dep)

            # 🛡️ FREN 1: İki takımın TOPLAMI minimum eşiği geçmeli
            toplam = oran_ev + oran_dep
            if toplam < MIN_TOPLAM_PUAN:
                continue

            # 🛡️ FREN 2: En az BİR takım çok güçlü eşleşmeli (0.80+)
            # Böylece "Aktobe - Lento" vs "Aktobe - Kaisar" karışmaz:
            # Ev aynı olsa bile deplasman zayıfsa toplam puan düşük kalır.
            if oran_ev < GUCLU_TAKIM_ESIGI and oran_dep < GUCLU_TAKIM_ESIGI:
                continue

            # 🛡️ FREN 3: Tek taraf süper, diğer taraf çöpse ELE
            # (örn: ev=1.00 ama dep=0.15 → bu farklı bir maçtır!)
            if min(oran_ev, oran_dep) < 0.35:
                continue

            puan = toplam  # Baz puan (max 2.0)

            # 🎯 BONUS 1: Tarih TAM aynıysa +0.50
            d1 = esnek_tarih_parse(mevcut_mac.get("tarih", ""))
            d2 = esnek_tarih_parse(sp_tarih)
            if d1 and d2 and d1 == d2:
                puan += 0.50
            elif d1 and d2 and abs((d1 - d2).days) == 1:
                puan += 0.20  # 1 gün fark: küçük bonus (gece yarısı maçları)

            # 🎯 BONUS 2: İki isim de birebir aynıysa +1.00
            if oran_ev >= 0.99 and oran_dep >= 0.99:
                puan += 1.00

            # 🎯 BONUS 3: Skor henüz yazılmamış (0-0, baslamadi) maça öncelik
            # Böylece zaten dolu maçın üstüne ikinci kez yazmak yerine boş olana yazar
            if mevcut_mac.get("durum", "") == "baslamadi":
                puan += 0.15

            if puan > en_iyi_puan:
                en_iyi_puan = puan
                en_iyi_index = idx
                en_iyi_detay = f"ev:{oran_ev:.2f} dep:{oran_dep:.2f}"

        # ─────────────────────────────────────────────
        # 2️⃣ EN İYİ ADAY VARSA GÜNCELLE
        # ─────────────────────────────────────────────
        if en_iyi_index >= 0:
            mevcut_mac = mac_listesi[en_iyi_index]
            kullanilan_indexler.add(en_iyi_index)
            ...
            if degisti_mi:
                ...
                guncelle_say += 1
                print(f"🔄 GÜNCELLE [P:{en_iyi_puan:.2f} | {en_iyi_detay}]: "
    ...
    print(f"\n📊 EŞLEŞTİRME ÖZETİ: 🔄 {guncelle_say} güncellendi | ➕ {ekle_say} yeni | ⏭️ {atlanan_say} zaten günceldi")
