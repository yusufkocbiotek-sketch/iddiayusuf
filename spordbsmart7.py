...
# Cloudflare / bot koruması için ek bekleme
CLOUDFLARE_BEKLEME = 8
...
# =========================
# 🚀 DRIVER — TÜM BAĞLANTI SORUNLARI DÜZELTİLDİ
# =========================
def build_driver():
    socket.setdefaulttimeout(30)  # 15'ten 30'a çıkarıldı
    opts = Options()

    # ── Temel kararlılık argümanları ──
    ...
    # ── ❌ KALDIRILDI: --disable-images (geçersiz argüman, sayfa yüklemeyi bozuyordu)
    # ── ❌ KALDIRILDI: --proxy-server='direct://' (tırnak hatası, bağlantıyı engelliyordu)

    # ── Resim yüklemeyi engelleme (DOĞRU YOL: prefs üzerinden) ──
    prefs = {
    ...
    # ── ❌ KALDIRILDI: page_load_strategy = "eager"
    #    "eager" JavaScript içerik yüklenmeden sayfa hazır sayıyordu.
    #    Spordb.com dinamik içerik yüklüyor, "normal" şart. ──
...
# =========================
# 🛡️ SAYFA YÜKLEME + CLOUDFLARE/BOT KORUMASI GEÇİŞİ
# =========================
def sayfa_yukle_ve_dogrula(driver, url, max_deneme=3):
    """
    Sayfayı yükler, Cloudflare challenge varsa bekler,
    data: URL'ye düşerse tekrar dener, cookie popup kapatır.
    """
    for deneme in range(max_deneme):
        try:
            print(f"🌐 Sayfa yükleniyor (deneme {deneme + 1}/{max_deneme}): {url}")
            driver.get(url)

            # Cloudflare / bot koruması geçmesi için bekle
            time.sleep(CLOUDFLARE_BEKLEME)

            # data: URL kontrolü — sayfa yüklenmediyse tekrar dene
            current_url = driver.current_url
            if current_url.startswith("data:") or current_url == "about:blank":
                print(f"⚠️ Sayfa 'data:' olarak döndü, {5} sn sonra tekrar deneniyor...")
                time.sleep(5)
                driver.get(url)
                time.sleep(CLOUDFLARE_BEKLEME)
                ...
                current_url = driver.current_url
                if current_url.startswith("data:") or current_url == "about:blank":
                    print(f"⚠️ Hâlâ data: URL — cookie temizleniyor ve yeniden yükleniyor...")
                    driver.delete_all_cookies()
                    time.sleep(2)
                    driver.get(url)
                    time.sleep(CLOUDFLARE_BEKLEME + 3)
            ...
            try:
                ...
                print(f"⚠️ Body bulunamadı, sayfa tam yüklenmemiş olabilir")
                driver.execute_script("window.stop();")
                time.sleep(3)

            # Cloudflare challenge sayfası kontrolü
            page_source_lower = driver.page_source[:3000].lower()
            if "just a moment" in page_source_lower or "checking your browser" in page_source_lower:
                print(f"🛡️ Cloudflare challenge algılandı, {15} sn bekleniyor...")
                time.sleep(15)
                # Challenge sonrası sayfanın yeniden yüklenip yüklenmediğini kontrol et
                page_source_lower = driver.page_source[:3000].lower()
                if "just a moment" in page_source_lower:
                    print(f"⚠️ Cloudflare hâlâ aktif, deneme {deneme + 1} başarısız")
            ...
            cookie_popup_kapat(driver)

            # Sayfa gerçekten yüklendi mi kontrol et
            final_url = driver.current_url
            if not final_url.startswith("data:") and "spordb" in final_url:
                print(f"✅ Sayfa başarıyla yüklendi: {final_url}")
                return True
            elif not final_url.startswith("data:"):
                # Farklı bir URL'ye yönlendiyse ama data: değilse, yine de kabul et
                print(f"✅ Sayfa yüklendi (yönlendirme): {final_url}")
                return True
            else:
                print(f"⚠️ Sayfa hâlâ data: URL, deneme {deneme + 1} başarısız")

        except TimeoutException:
            print(f"⏱️ Zaman aşımı (deneme {deneme + 1}), sayfa durdurulup tekrar deneniyor...")
            try:
                driver.execute_script("window.stop();")
            except:
                pass
            ...
            print(f"⚠️ WebDriver hatası (deneme {deneme + 1}): {str(e)[:60]}")
            time.sleep(5)
        except Exception as e:
            print(f"❌ Beklenmeyen hata (deneme {deneme + 1}): {str(e)[:60]}")
            time.sleep(3)

    print("❌ Sayfa yüklenemedi, tüm denemeler tükendi")
    return False

# =========================
# 🍪 COOKIE / POPUP KAPATMA
# =========================
def cookie_popup_kapat(driver):
    """Cookie onay butonlarını ve overlay popup'ları kapatır."""

    # Yaygın cookie kabul butonları CSS seçicileri
    cookie_seciciler = [
        # Genel cookie butonları
        "button[id*='cookie' i]",
    ...
    for secici in cookie_seciciler:
        try:
            elements = driver.find_elements(By.CSS_SELECTOR, secici)
            for el in elements:
                if el.is_displayed():
                    el.click()
                    print(f"🍪 Popup/cookie kapatıldı: {secici}")
                    time.sleep(1)
                    return True
        except:
            continue

    # JavaScript ile overlay kaldırma
    try:
        driver.execute_script("""
            // Cookie banner'ları kaldır
            document.querySelectorAll('[class*="cookie"], [class*="consent"], [id*="cookie"], [id*="consent"]').forEach(function(el) {
                if (el.offsetHeight > 0) { el.style.display = 'none'; }
            });
            // Overlay'ları kaldır
            document.querySelectorAll('.overlay, .modal-backdrop').forEach(function(el) {
                el.style.display = 'none';
            });
            // Body scroll'u aç (bazen overlay body scroll'u kilitler)
...
def select_week(driver, hedef_tarih):
    ...
    try:
        # Elementin hem görünür hem de etkileşilebilir olmasını bekle
        select_el = WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "#iddaa_daterange"))
        )
        ...
        for opt in select_obj.options:
            ...
            try:
                ...
                if bas <= hedef_tarih <= bit:
                    ...
                    # Seçim sonrası sayfa içeriğinin güncellenmesini bekle
                    try:
                        WebDriverWait(driver, 20).until(
                            EC.presence_of_element_located((By.CSS_SELECTOR, "#iddaa_dateselector"))
                        )
                    except TimeoutException:
                        time.sleep(3)
...
def get_day_option(driver, hedef_tarih):
    try:
        select_el = WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "#iddaa_dateselector"))
        )
        select_obj = Select(select_el)
        for opt in select_obj.options:
    ...
    for deneme in range(MAX_DENEME):
        try:
            ...
            if driver.current_url.startswith("data:"):
                print(f"⚠️ {sira:2d}. Detay sayfası data: olarak döndü, atlanıyor...")
                raise Exception("data: URL")

            time.sleep(BEKLEME_ORTA)
            page_source = driver.page_source

            # Cloudflare kontrolü
            if "just a moment" in page_source[:2000].lower():
                print(f"🛡️ {sira:2d}. Cloudflare, 10sn bekleniyor...")
                time.sleep(10)
                page_source = driver.page_source
            ...
            if not h1 or " - " not in h1:
                ...
                if len(takim_linkleri) >= 2:
                    t1 = re.sub(r"<[^>]+>", "", takim_linkleri[0]).strip()
                    t2 = re.sub(r"<[^>]+>", "", takim_linkleri[1]).strip()
                    h1 = f"{t1} - {t2}"
            ...
            skor_match = re.search(r'id=["\']matchdetailscore["\'].*?>(\d+)\s*-\s*(\d+)</', page_source, re.DOTALL)
            if skor_match:
                data["skor_ev"] = int(skor_match.group(1))
                data["skor_dep"] = int(skor_match.group(2))

            iy_match = re.search(
                r'id=["\']matchdetailhtscore["\'].*?IY\s*(\d+)\s*-\s*(\d+)\s*</',
            ...
            if "maç sonu" in page_lower or "bitti" in page_lower or skor_match:
                data["durum"] = "bitti"
            elif "devam" in page_lower:
                data["durum"] = "devam"

            print(f"✅ {sira:2d}. | {data['ev_sahibi']} - {data['deplasman']} | "
                  f"SKOR: {data['skor_ev']}-{data['skor_dep']} | "
                  f"IY: {data['skor_1y_ev']}-{data['skor_1y_dep']}")
            break
            ...
            print(f"⚠️ {sira:2d}. | DENEME {deneme + 1}: {str(e)[:40]}")
            time.sleep(2)
        finally:
            try:
                ...
                try:
                    # Window handle'lar bozulmuşsa sıfırla
...
def fetch_day(driver, gun):
    ...
    try:
        ...
        sel = WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "#iddaa_dateselector"))
        )
        Select(sel).select_by_value(gun["deger"])

        # İçeriğin güncellenmesini bekle
        time.sleep(BEKLEME_UZUN)
        ...
        try:
            ...
            print(f"⚠️ Maç linkleri bulunamadı, sayfa kaynağı kontrol ediliyor...")
            # Sayfada veri var mı kontrol et
            if "data:" in driver.current_url:
                print("❌ Sayfa data: URL, gün atlanıyor")
                return matches
        ...
        linkler = []
        for elem in link_elemanlari:
            try:
                href = elem.get_attribute("href")
                if href and "-maci-" in href and href not in linkler:
                    linkler.append(href)
        ...
        for i, href in enumerate(linkler, 1):
            try:
                ...
                print(f"❌ {i}. Maç genel hata: {str(e)[:40]}")
                continue

    except Exception as e:
        print(f"❌ GÜN GENEL HATA: {str(e)[:60]}")
    return matches
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 SPORDB | BAĞLANTI SORUNLARI DÜZELTİLDİ ✅")
    print("=" * 60)
    ...
    try:
        ...
        for gun_tarihi in hedef_gunler:
            ...
            try:
                if not driver:
                    print("🔄 Driver yeniden oluşturuluyor...")
                    driver = build_driver()

                # ── YENİ: sayfa_yukle_ve_dogrula kullan ──
                if not sayfa_yukle_ve_dogrula(driver, SPORDB_URL):
                    print(f"⏭️ {gun_tarihi} atlandı (sayfa yüklenemedi)")
                    # Driver'ı sıfırlayıp bir sonraki gün dene
                    try:
                        driver.quit()
                    except:
                        pass
                ...
                print(f"🔌 Bağlantı koptu ({str(e)[:40]}), yeniden başlatılıyor...")
                if driver:
                    try:
                        driver.quit()
                    except:
                ...
                print(f"❌ {gun_tarihi} işlenirken hata: {str(gun_hata)[:60]}")
                traceback.print_exc()
                continue
