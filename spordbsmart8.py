...
# =========================
# 🚀 DRIVER - `data:,` SORUNU ÇÖZÜLDÜ ✅
# =========================
def build_driver():
    opts = Options()
    # Temel güvenlik/hız ayarları
    ...
    # ✅ ÖNEMLİ: Sayfa yükleme stratejisi DEĞİŞTİRİLDİ (none → normal)
    ...
    try:
        ...
        driver.set_script_timeout(SCRIPT_SURESI)
...
def select_week(driver, hedef_tarih):
    ...
    try:
        select_el = WebDriverWait(driver, 25).until(
...
def fetch_day(driver, gun):
    ...
    try:
        ...
        sel = WebDriverWait(driver, 25).until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, "#iddaa_dateselector")))
...
def merge_data(mevcut, yeni):
    ...
    print(f"\n📊 ÖZET: 🔄 {guncelle_say} güncelleme | 👯 {kopya_say} kopya | ➕ {ekle_say} yeni")
...
def git_islemleri():
    try:
        ...
        commit_mesaji = f"Veri Güncelleme | {zaman}"
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 SPORDB | data:, SORUNU ÇÖZÜLDÜ ✅")
    print("=" * 60)
    ...
    try:
        ...
        for gun_tarihi in hedef_gunler:
            ...
            try:
                ...
                print(f"🌐 Sayfa yükleniyor: {SPORDB_URL}")
                driver.get(SPORDB_URL)
                # ✅ Yükleme takibi
                WebDriverWait(driver, SAYFA_YUKLEME_SURESI).until(
                    lambda d: d.current_url != "data:" and d.current_url.startswith("https://www.spordb.com/")
                )
                print(f"✅ Sayfa açıldı: {driver.current_url}")
                time.sleep(BEKLEME_UZUN)

                if not select_week(driver, gun_tarihi):
                    print(f"⏭️ {gun_tarihi} atlandı")
                    continue
                time.sleep(BEKLEME_ORTA)
                ...
                print(f"📊 {gun['gorunen']} → {len(gunluk)} maç")

            except (InvalidSessionIdException, WebDriverException, ConnectionError, TimeoutException) as e:
                print(f"🔌 Bağlantı/sayfa hatası: {e} → Yeniden başlatılıyor...")
                if driver:
                    try: driver.quit()
                    except: pass
                driver = None
        ...
        print(f"\n📊 TOPLAM: {len(tum_yeni)} maç çekildi")
        if tum_yeni:
            mevcut_veri = load_json_safe(MAC_JSON_PATH)
            yeni_veri, guncelle, ekle = merge_data(mevcut_veri, tum_yeni)
            if save_json_safe(yeni_veri, MAC_JSON_PATH):
                print(f"\n🎉 Tamamlandı: {guncelle} güncelleme | {ekle} yeni")
                git_islemleri()
        else:
            print("\n⚠️ Veri çekilemedi")
