...
ULTRASURF_EXE = "u_122145.exe"  # Ultrasurf dosyasının adı (kendine göre değiştir)
...
def ultrasurf_baslat():
    """Ultrasurf'ü başlat (yeni IP alır)."""
    ...
    try:
        ...
        time.sleep(12)  # Bağlantı kurulsun
        print("   ✅ Ultrasurf başlatıldı (yeni IP)")
    except Exception as e:
        print(f"   ⚠️ Ultrasurf hatası: {e}")

def ultrasurf_durdur():
    """Ultrasurf'ü durdur."""
    print("   🛑 Ultrasurf durduruluyor...")
    try:
...
def tarayici_baslat(proxy=True):
    """Chrome'u başlat (Ultrasurf proxy ile veya proxy'siz)."""
...
def mac_cek():
    ...
    try:
        ...
        # Proxy'li tarayıcı başlat
        driver = tarayici_baslat(proxy=True)
        ...
        for idx, mac in enumerate(mac_listesi):
            # Her SESSION_LIMIT maçta bir IP DEĞİŞTİR + TARAYICI YENİLE
            if session_sayaci >= SESSION_LIMIT:
                print(f"\n      🔄 IP DEĞİŞTİRİLİYOR ({SESSION_LIMIT} maç tamamlandı)")
                ...
                # Ultrasurf'ü yeniden başlat (yeni IP)
                ip_degistir()
                
                # Yeni proxy'li tarayıcı aç
                driver = tarayici_baslat(proxy=True)
                driver.get(url)
                rastgele_bekle(8, 12)
                session_sayaci = 0
                print(f"      ✅ Yeni session başlatıldı!\n")
